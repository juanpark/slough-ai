...
| 증분/전체 학습 | `/slough-ingest`로 새 메시지 추가 학습, `/slough-ingest full`로 전체 재학습 |
...
└──────┬───────┘
       ...
       ▼
┌──────────────┐
│   retrieve   │ ──── 쿼리 리라이팅 (GPT-4o-mini) + 멀티쿼리 pgvector 검색 (top-k=8, threshold=0.3, 날짜 태그)
└──────┬───────┘
       │
       ▼
┌──────────────┐
│   generate   │ ──── GPT-4o (temperature=0.3) + 근거 기반 규칙 + 페르소나 + 3-layer 메모리
└──────────────┘
```

### 주요 특징

- **쿼리 리라이팅:** GPT-4o-mini가 사용자 질문을 2-3개의 검색 최적화된 변형 쿼리로 확장 → 검색 재현율 향상
- **유사도 임계값 (0.3):** 관련 없는 문서 필터링, 멀티쿼리 결과 중복 제거 후 top-k=8
- **관련성 라벨링 + 날짜 태그:** 검색된 문서에 점수 기반 `[높은 관련성]`, `[관련성 있음]`, `[낮은 관련성]` 라벨 + `[YYYY-MM-DD]` 날짜 태그 부여
- **시간 인식 답변:** 검색 결과에 날짜를 포함하여 "최근 결정은?" 같은 시간 기반 질문에 정확한 답변 가능
- **인용 태그 체계:** 실제 발언 기반 답변에 `[발언 근거]`, 추론 기반 답변에 `[추론]` 태그를 강제하여 근거 투명성 확보
- **프롬프트 구조 최적화:** LLM의 "recency bias" 활용 — 검색된 문맥과 근거 규칙을 프롬프트 끝에 배치하여 참조 정확도 향상
- **근거 기반 답변:** 실제 발언을 인용/패러프레이즈하도록 강제, 학습되지 않은 내용 생성 방지
- **시간 가중치 검색:** 최신 메시지를 우선 반영 (`1 / (1 + 0.1 * ln(age_days + 1))`)
- **페르소나 자동 추출:** GPT-4o-mini가 50개 샘플 메시지 분석 → 말투/성격/의사결정 스타일/관심 주제/대표 발언 원문 추출
...
- **짧은 답변 맥락 보존:** 80자 미만 짧은 메시지는 앞선 2-3개 메시지를 `[맥락]...[의사결정자 답변]...` 형태로 함께 저장
